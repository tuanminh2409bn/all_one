...
Last updated: 2026-09-23 (Asia/Ho_Chi_Minh)
...
The iOS/TestFlight build number is now `3` (`pubspec.yaml`: `1.0.0+3`) for the next Xcode Cloud archive. The local `pubspec.lock` regenerated by a newer Flutter/Dart SDK is intentionally not part of this release because Xcode Cloud pins Flutter 3.38.10. Before the push, the full serial `flutter test --no-pub --concurrency=1` suite passed 67 tests. `flutter analyze --no-pub` reported only the existing `ReorderableListView.onReorder` deprecation from the newer local Flutter SDK. The two affected Loading 2 confirmation golden baselines were visually reviewed and regenerated for the intentionally revised confirmation layout.
...
- Failed verification locks on the fifth wrong attempt. The login PIN screen retains Firebase reauthentication for reset; the transfer PIN popup does not expose a reset action.
...
- The Home `오늘의 혜택` card now uses `assets/images/home_daily_benefit_loop.png` instead of a static hand icon/text row. The 410×70 APNG crops only the icon/text strip from the supplied `Downloads/1.mp4` at source pixels `(56, 564)`; its 175 frames at 20 fps cover one 8.75-second three-message cycle and loop indefinitely. The card layout and other Home artwork remain native. Its source-video raster is visibly softer than native text on high-density screens. Home and overlay golden baselines showing this strip were visually reviewed and updated. The full serial test suite passes 70 tests; `flutter analyze --no-pub` still reports only the unrelated `onReorder` deprecation info.
...
- Home `거래내역` now plays the existing Loading 2 APNG over a `0x7E000000` scrim before opening `AccountDetailsScreen`. The supplied `Downloads/3.mp4` shows the dimmed Home first, a switch to the dimmed history page near the end, then removal of the loader. This transition lasts 1500 ms, switches the backdrop at 1300 ms, and retains the 120×120 reference logo box centered at `(234, 581)` on the 588×1280 canvas. Reviewed checkpoints are `preview_9c_home_history_loading_home.png` and `preview_9d_home_history_loading_details.png`.
- The new timing/navigation widget test and both loading goldens pass. The full serial `flutter test --no-pub --concurrency=1` suite passes 69 tests, including the four earlier transfer loadings. `flutter analyze --no-pub` reports only the existing unrelated `ReorderableListView.onReorder` deprecation info from the newer local Flutter SDK. No Android or iOS build was run for this change.
...
- Recent recipient rows use the catalog bank label/logo, the original lighter subtitle treatment, and a custom five-point favorite mark. The favorite is a gray outline when inactive and the sampled original green fill when active; tapping it persists the recipient favorite state. The `삭제하기` link appears only when at least one recent recipient is starred and disappears when none is starred.
- On the manual recipient screen, the starred Recent tab keeps `삭제하기` and recipient rows below the tab underline in both bank-selection states. When no bank is selected, these controls move down with the tabs instead of overlapping `연락처`; without a starred recipient, the list moves up to fill the management-link space.
- The corrected no-bank populated-Recent states are captured in `test/goldens/preview_11b0_transfer_recipient_recent.png` (starred) and `test/goldens/preview_11b0_transfer_recipient_recent_unstarred.png` (unstarred) for visual comparison.
- The amount-entry source-account card displays the selected account's current `AppDataStore.balanceFor(account.id)` balance, including saved transaction deltas. Entering a transfer amount does not change this displayed balance; changing source accounts or updating the account balance does. The transfer loading/warning backdrop continues to show the transfer amount in its separate source card. The reviewed amount checkpoints are `preview_11d_transfer_amount_empty.png` and `preview_11e_transfer_amount_5000.png`.
- On the amount-entry page only, the underlined recipient bank/account line uses `#707070` for both glyphs and underline, following the circled reference in `Downloads/2.jpg`. The recipient name and the shared identity shown in PIN/warning states retain their prior black treatment. The two amount golden checkpoints were refreshed and visually reviewed; the focused signed-in transfer widget test passes.
...
- Saved-recipient transfer amount/PIN fidelity pass on 2026-09-21:
  ...
  - The green `비밀번호 재설정` action was removed from the transfer PIN popup after a failed attempt; the post-error keypad is captured in `preview_11g1_transfer_pin_after_failure.png`.
...
- Per-recipient transfer-warning and confirmation flow on 2026-09-21:
  ...
  - The confirmation page now draws only its own close `X` (the shared top controls no longer paint a second one), uses `assets/images/ref_transfer_review_edit.png` cropped from the supplied original-app screenshot for all three pencil icons, increases the two-line blue fee copy's line spacing, and restores the original muted treatment only for the bottom `메모` row and placeholder. The focused widget check and updated/reviewed `preview_11i_transfer_confirmation.png` golden pass; analysis reports only the existing unrelated `onReorder` deprecation.
- Transfer-failure popup fidelity pass on 2026-09-21:
  ...
  - The original header asset was `assets/images/ref_transfer_failure_brand.jpg`, a 164×44 JPEG crop from the supplied original-app capture. The popup still uses the same 164×44 design box.
  - A 2026-09-23 sharpness follow-up replaced only that popup's active artwork with `assets/images/ref_transfer_failure_brand_sharp.png` (1600×351), rasterized from page 1 of NH농협은행's official vector CI signature package at `https://www.nhbank.com/upload/files/ai01.zip`. The artwork matches the supplied yellow/blue reference mark without upscaling the 164×44 JPEG; the focused widget test locks in the new asset path. The refreshed `preview_11j_transfer_failure_home.png` was reviewed and passed focused golden comparison; analysis reports only the existing unrelated `onReorder` deprecation.
